...
# Specify the allowed origin for your React dev server
origins = [
    "http://localhost:5173",
...
def extract_json_from_markdown(text):
    ...
    try:
        ...
        if start_index == -1:
            # If no json block is found, assume the entire text is the JSON
            return text
        
        end_index = text.find('```', start_index + 1)
        if end_index == -1:
            # If the block is not closed, assume it's to the end of the text
            end_index = len(text)
...
async def analyze_code(request: Request):
    ...
    try:
        ...
        # Extract the JSON content from the Markdown wrapper
